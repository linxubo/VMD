  ...
  # We modified the script for recognizing "#include "toppar/A.itp"". #
    ...
    if [eof $top] break
     if { [string bytelength $line] > 0 && [string first ";" [string trim $line] 0] != 0 } {
      ...
      if { [string first "#include" $line 0] > -1 } {
        ...
        if { [file pathtype $itp] == "relative" } {
          # If relative, prepend the directory of the top file
          set itp [file join $path $itp]
        }
        # Check if file exists
        file_exists $itp
        # Parse the itp file
